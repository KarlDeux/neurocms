...
/*
 *  @Special thanks to Sebastian Tschan
 *  @author   Carlos Lizaga
 *  @copyright   2013-2014 Imprentaonline S.L.
 *  @versio0.1
 *  @license All rights reserved.
 */

include_once(getenv("DOCUMENT_ROOT") . '/' . getenv("ROOT") . 'config/base.config.php');
include_once(getenv("DOCUMENT_ROOT") . '/' . getenv("ROOT") . 'config/definitions.config.php');
include_once(getenv("DOCUMENT_ROOT") . '/' . getenv("ROOT") . 'objects/MySqlDatabase.php');
...
class UploadHandler {
	...
	$this->options = array(
	    ...
	    'inline_file_types' => '/\.(jpe?g|png|pdf|zip|rar|ai|tiff)$/i',
	...
	if (!$direct && $this->options['download_via_php']) {
	    $url = $this->options['script_url']
		    . 'index.php?folder=' . $this->options['folder'] . '&file=' . rawurlencode($file_name);
	    if ($version) {
	...
	$file->deleteUrl = $this->options['script_url']
		...
		. 'index.php?folder=' . $this->options['folder'] . '&file=' . rawurlencode($file->name);
	...
	if (!is_dir($upload_dir)) {
        ...
        $result = $this->db->executeQuery("select file_name from " . _DB_PREFIX_ . "uploaded_files where folder ='" . $this->options['folder'] . "' order by id asc");

        while ($row = $result->fetch_assoc()) {  

            $savedArray[] = $row["file_name"];
        }
       	
	return array_values(array_filter(array_map(array($this, $iteration_method), $savedArray)));
	...
	$name = ($this->options['folder']) . "-" . substr(md5($name), 0, 5) . "_" . date('d-m-Y_H-i-s') . "." . $extension[sizeof($extension) - 1];
	...
	if (strpos($name, '.') === false &&
        ...
        $this->db->executeQuery('insert into ' . _DB_PREFIX_ . 'uploaded_files (folder, file_name) values ("' . $this->options['folder'] . '", "' . $name . '")');
	...
	$success = is_file($file_path) && $file_name[0] !== '.' && unlink($file_path);
        $this->db->executeQuery('delete from ' . _DB_PREFIX_ . 'uploaded_files where file_name = "' . $file_name . '"');
	if ($success) {
	    foreach ($this->options['image_versions'] as $version => $options) {
		if (!empty($version)) {
